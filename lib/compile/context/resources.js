 ...
 * @return {String}
 */
var toVariableFormat = function (uri, spec) {
...
var attachResource = function (obj, resource, spec) {
  ...
  (function recurse (obj, parts) {
    ...
    if (part !== '/') {
      ...
      var variableName = toVariableFormat(part, spec);

      // TODO: Handle duplicate variable names and missing variable names.
      if (!variableName || obj.children.hasOwnProperty(variableName)) {
        // Handle exact matching relative uris. If they don't match exactly,
        // it's impossible for us to handle generically.
        if (part === obj.children[variableName].relativeUri) {
          obj = obj.children[variableName];
        } else {
          return;
        }
      } else {
        // Extract the uri parameters in their array order.
        var uriParameters = _.map(part.match(TEMPLATE_REGEXP), getParamTag);

        // Create or reuse current the object.
        obj = obj.children[variableName] = {
          id:            uniqueId(),
          parent:        obj,
          children:      {},
    ...
    obj.methods = sanitizeMethods(resource.methods, spec);
